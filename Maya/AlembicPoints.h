...
class AlembicPoints : public AlembicObject {
 private:
  ...
  Abc::OStringArrayProperty
      mInstanceNamesProperty;  // only written once, when mNumSample == 0
  ...
  bool sampleInstanceProperties(std::vector<Abc::Quatf> angularVel,
                                std::vector<Abc::Quatf> orientation,
                                std::vector<Abc::uint16_t> shapeId,
                                std::vector<Abc::uint16_t> shapeType,
                                std::vector<Abc::float32_t> shapeTime);

 public:
  AlembicPoints(SceneNodePtr eNode, AlembicWriteJob *in_Job,
                Abc::OObject oParent);
  ~AlembicPoints();
...
class BasePropertyManager {
 public:
  ...
  BasePropertyManager(const std::string &name,
