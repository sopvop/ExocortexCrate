...
bool isRefAnimated(const MObject& in_Ref)
{
  ...
  // gIsRefAnimatedMap.find(in_Ref.GetAsText().asChar());
...
void nameMapAdd(MString identifier, MString name)
{
  ...
  if (it == gNameMap.end()) {
    std::pair<std::string, std::string> pair(identifier.asChar(),
                                             name.asChar());
...
void nameMapClear() { gNameMap.clear(); }
...
MStatus AlembicResolvePathCommand::doIt(const MArgList& args)
{
  ...
  if (argData.isFlagSet("help")) {
    MGlobal::displayInfo(
        "[ExocortexAlembic]: ExocortexAlembic_resolvePath command:");
    MGlobal::displayInfo(
