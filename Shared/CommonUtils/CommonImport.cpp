...
bool IJobStringParser::parse(const std::string& jobString)
{
  ...
  for (int j = 0; j < tokens.size(); ++j) {
    ...
    if (boost::iequals(valuePair[0], "prefix")) {
      ...
      selectShapes = parseBool(valuePair[1]);
             ...
             boost::iequals(valuePair[0], "identifiers")) {
  ...
  if (search_str.length() ? !replace_str.length()
                          : replace_str.length())  // either search or replace
                                                   // string is missing or
                                                   // empty!
  {
    ESS_LOG_WARNING(
        "Missing search or replace parameter. No strings will be replaced.");
...
{
  ////Note: there are currently some minor differences between the apps. This
  ///function is somewhat XSI specific.
  ...
  stream << "normals=" << importNormals << ";uvs=" << importUVs
         << ";facesets=" << importFacesets;
  stream << ";importVisibilityControllers=" << importVisibilityControllers
         << ";importTransformControllers=" << importTransformControllers;
  stream << ";importStandinProperties=" << importStandinProperties;
  stream << ";importBoundingBoxes=" << importBoundingBoxes
         << ";attachToExisting=" << attachToExisting
         << ";skipUnattachedNodes=" << skipUnattachedNodes;
  stream << ";failOnUnsupported=" << failOnUnsupported
         << ";enableImportRootSelection=" << enableImportRootSelection
         << ";stripMayaNamespaces=" << stripMayaNamespaces;
  stream << ";importCurvesAsStrands=" << importCurvesAsStrands
         << ";enableSubD=" << enableSubD << ";useMultifile=" << useMultiFile;
  stream << ";operatorCreationForExistingNodes="
         << operatorCreationForExistingNodes;
  ...
  for (std::map<std::string, std::string>::iterator beg =
           extraParameters.begin();
       beg != extraParameters.end(); ++beg)
...
SceneNode::nodeTypeE getNodeType(Alembic::Abc::IObject iObj)
{
  ...
  if (AbcG::IXform::matches(metadata)) {
    ...
    // Note: this method assumes everything is an ITRANSFORM, this is corrected
    // later on in the the method that builds the common scene graph
  }
...
struct AlembicISceneBuildElement {
  ...
  AlembicISceneBuildElement(AbcObjectCache* pMyObjectCache, SceneNodePtr node,
                            bool directChild = false)
      : pObjectCache(pMyObjectCache),
        parentNode(node),
        bIsDirectChild(directChild)
  {
  }
};

SceneNodeAlembicPtr buildAlembicSceneGraph(AbcArchiveCache* pArchiveCache,
                                           AbcObjectCache* pRootObjectCache,
                                           int& nNumNodes,
                                           const IJobStringParser& jobParams,
                                           bool countMergableChildren,
                                           CommonProgressBar* pBar)
{
  ...
  for (size_t j = 0; j < pRootObjectCache->childIdentifiers.size(); j++) {
    if (pBar && j % 20 == 0) {
      pBar->incr(1);
      if (pBar->isCancelled()) return SceneNodeAlembicPtr();
    }
    ...
    // we should change this to explicity check which node types are not support
    // (e.g. facesets), so that we can still give out warnings
    if (childCat == NodeCategory::UNSUPPORTED)
      continue;  // skip over unsupported types

    sceneStack.push_back(
        AlembicISceneBuildElement(pChildObjectCache, sceneRoot, true));
  }
  ...
  while (!sceneStack.empty()) {
    ...
    if (newNode->type == SceneNode::ITRANSFORM) {
      ...
      for (int j = 0; j < (int)sElement.pObjectCache->childIdentifiers.size();
           j++) {
        AbcObjectCache* pChildObjectCache =
      ...
      if (geomNodeCount == 1) {  // the xform has only one geometry, child so it
                                 // is possible to merge. Thus, this is an
                                 // ETRANFORM.
    ...
    std::vector<std::string>::iterator
        chIter = sElement.pObjectCache->childIdentifiers.begin(),
        chEnd = sElement.pObjectCache->childIdentifiers.end();
    for (; chIter != chEnd; ++chIter) {
      AbcObjectCache* pChildObjectCache =
          &(pArchiveCache->find(*chIter)->second);
      Alembic::AbcGeom::IObject childObj = pChildObjectCache->obj;
      NodeCategory::type childCat = NodeCategory::get(childObj);
      // we should change this to explicity check which node types are not
      // support (e.g. facesets), so that we can still give out warnings
      if (childCat == NodeCategory::UNSUPPORTED)
        continue;  // skip over unsupported types

      sceneStack.push_back(
          AlembicISceneBuildElement(pChildObjectCache, newNode));
    }
  }
...
bool validateSceneFileAttached(SceneNodeAlembicPtr fileRoot,
                               bool bSkipUnattachedNodes)
{
  ESS_PROFILE_FUNC();
  std::list<ValidateStackElement> sceneStack;

  for (SceneChildIterator it = fileRoot->children.begin();
       it != fileRoot->children.end(); it++) {
    SceneNodeAlembicPtr fileNode =
        reinterpret<SceneNode, SceneNodeAlembic>(*it);
  ...
  while (!sceneStack.empty()) {
    ...
    if (!currFileNode->isAttached() /*&& currFileNode->selected*/) {
      if (bSkipUnattachedNodes) {
        ESS_LOG_WARNING(
            "Node failed to attach: " << currFileNode->dccIdentifier);
      }
      else {
    ...
    for (SceneChildIterator it = currFileNode->children.begin();
         it != currFileNode->children.end(); it++) {
      SceneNodeAlembicPtr fileNode =
          reinterpret<SceneNode, SceneNodeAlembic>(*it);
...
//                  /*SCENE_ROOT*/ /*ETRANSFORM*/ /*ITRANSFORM*/ /*CAMERA*/
//                  /*POLYMESH,*/ /*SUBD*/ /*SURFACE*/ /*CURVES*/ /*PARTICLES*/
//                  /*HAIR*/ /*LIGHT*/ /*UNKNOWN*/
//   //we never actually import the scene root. we have to use the application
//   scene root.
//   /*SCENE_ROOT*/ {false,            false,         false,         false,
//   false,     false,    false,      false,      false,      false,   false,
//   false},
//   //E
///*ETRANSFORM*/    {false,            false,         false,         false,
///false,     false,    false,      false,      false,      false,   false,
///false},
///*ITRANSFORM*/    {false,            false,         true,          false,
///false,     false,    false,      false,      false,      false,   false,
///false},
///*CAMERA*/
///*POLYMESH,*/
...
NodeMapPtr buildChildMap(SceneNodeAlembicPtr parent)
{
  ...
  for (SceneChildIterator it = parent->children.begin(); it != endIt; it++) {
    ...
    // Note: this algorithm assumes that parents of each selected node are also
    // selected
    // if(!node->selected){
    //   continue;
    //}

    // for AttachToScene. Geometry nodes that have been merged with a parent
    // transform should be skipped.
    if (node->isMerged()) {
      continue;
    }
    ...
    if (name.size() == 0) {
      ESS_LOG_WARNING("Warning: node name is empty. Cannot add \""
                      << name << "\" to map.");
    }

    if (map->find(name) != map->end()) {
      ESS_LOG_WARNING("Warning: duplicate node name. Cannot add \""
                      << name << "\" to map.");
    }
...
struct AttachStackElement {
  ...
  AttachStackElement(SceneNodeAppPtr appNode, NodeMapPtr mapPtr)
      : currAppNode(appNode), childMapPtr(mapPtr)
  {
  }
};

bool AttachSceneFile(SceneNodeAlembicPtr fileRoot, SceneNodeAppPtr appRoot,
                     const IJobStringParser& jobParams, CommonProgressBar* pbar,
                     std::list<SceneNodeAppPtr>* newNodes)
{
...

  {
    ...
    for (SceneChildIterator it = appRoot->children.begin();
         it != appRoot->children.end(); it++) {
  ...
  while (!sceneStack.empty()) {
    ...
    const std::string& appNodeNameWithPrefix =
        removeXfoSuffix(currAppNode->name);
    ...
    if (!jobParams.prefix.empty()) {
      ...
      if (pos == std::string::npos) {
        ...
        appNodeName = appNodeNameWithPrefix.substr(
            jobParams.prefix.length(), appNodeNameWithPrefix.length());
      }
    }
    else {
      appNodeName = appNodeNameWithPrefix;
    ...
    if (fileNodeIt != childMapPtr->end()) {  // we have a match
      ...
      if (fileNode->isAttached()) {
        ...
        bChildAttached =
            currAppNode->replaceData(fileNode, jobParams, ignoreFileNode);
    ...
    // push the children as the last step, since we need to who the parent is
    // first (we may have merged)
    for (SceneChildIterator it = currAppNode->children.begin();
         it != currAppNode->children.end(); it++) {
      SceneNodeAppPtr appNode = reinterpret<SceneNode, SceneNodeApp>(*it);
...
struct ImportStackElement {
  ...
  ImportStackElement(SceneNodeAlembicPtr node, SceneNodeAppPtr parent)
      : currFileNode(node), parentAppNode(parent)
  {
  }
};

bool ImportSceneFile(SceneNodeAlembicPtr fileRoot, SceneNodeAppPtr appRoot,
                     const IJobStringParser& jobParams, CommonProgressBar* pbar,
                     std::list<SceneNodeAppPtr>* newNodes)
{
  ESS_PROFILE_FUNC();

  // compare to application scene graph to see if we need to rename nodes (or
  // maybe we might throw an error)
  ...
  while (!sceneStack.empty()) {
    ...
    // ESS_LOG_WARNING("Importing
    // "<<currFileNode->pObjCache->obj.getFullName());

    SceneNodeAppPtr newAppNode;
    bool bContinue =
        parentAppNode->addChild(currFileNode, jobParams, newAppNode);
    ...
    if (newAppNode) {
      // ESS_LOG_WARNING("newAppNode: "<<newAppNode->name<<" useCount:
      // "<<newAppNode.use_count());
      ...
      for (SceneChildIterator it = currFileNode->children.begin();
        ...
        if (fileNode->isMerged()) {
          // The child node was merged with its parent, so skip this child, and
          // add its children
          //(Although this case is technically possible, I think it will not be
          //common)
          SceneNodePtr& mergedChild = *it;

          for (SceneChildIterator cit = mergedChild->children.begin();
               cit != mergedChild->children.end(); cit++) {
            SceneNodeAlembicPtr cfileNode =
                reinterpret<SceneNode, SceneNodeAlembic>(*cit);
      ...
      if (currFileNode->children.empty() == false) {
        EC_LOG_WARNING("Unsupported node: "
                       << currFileNode->name
                       << " has children that have not been imported.");
      }
    }
  }
...
template <class T>
void GetSampleRangeT(Abc::IObject& obj, std::size_t& oMinSample,
                     std::size_t& oMaxSample, double& oMinTime,
                     double& oMaxTime)
{
  T xObj(obj, Abc::kWrapExisting);
  Alembic::AbcCoreAbstract::TimeSamplingPtr ts =
      xObj.getSchema().getTimeSampling();
...
void GetSampleRange(SceneNodeAlembicPtr fileRoot, std::size_t& oMinSample,
                    std::size_t& oMaxSample, double& oMinTime, double& oMaxTime)
{
  ESS_PROFILE_FUNC();
  ...
  while (!sceneStack.empty()) {
    ...
    SceneNodeAlembicPtr alembicNode =
        reinterpret<SceneNode, SceneNodeAlembic>(currFileNode);
    Abc::IObject obj = alembicNode->getObject();

    AbcG::MetaData metadata = obj.getMetaData();
    if (AbcG::IXform::matches(metadata)) {
      GetSampleRangeT<AbcG::IXform>(obj, oMinSample, oMaxSample, oMinTime,
                                    oMaxTime);
    }
    else if (AbcG::IPolyMesh::matches(metadata)) {
      GetSampleRangeT<AbcG::IPolyMesh>(obj, oMinSample, oMaxSample, oMinTime,
                                       oMaxTime);
    }
    else if (AbcG::ISubD::matches(metadata)) {
      GetSampleRangeT<AbcG::ISubD>(obj, oMinSample, oMaxSample, oMinTime,
                                   oMaxTime);
    }
    else if (AbcG::ICamera::matches(metadata)) {
      GetSampleRangeT<AbcG::ICamera>(obj, oMinSample, oMaxSample, oMinTime,
                                     oMaxTime);
    }
    else if (AbcG::IPoints::matches(metadata)) {
      GetSampleRangeT<AbcG::IPoints>(obj, oMinSample, oMaxSample, oMinTime,
                                     oMaxTime);
    }
    else if (AbcG::ICurves::matches(metadata)) {
      GetSampleRangeT<AbcG::ICurves>(obj, oMinSample, oMaxSample, oMinTime,
                                     oMaxTime);
    }
    else if (AbcG::ILight::matches(metadata)) {
      GetSampleRangeT<AbcG::ILight>(obj, oMinSample, oMaxSample, oMinTime,
                                    oMaxTime);
    }
    else if (AbcG::INuPatch::matches(metadata)) {
      GetSampleRangeT<AbcG::INuPatch>(obj, oMinSample, oMaxSample, oMinTime,
                                      oMaxTime);
    }
    else {
      ESS_LOG_WARNING(
          "Warning: time range cannot be read for unrecoginized type.");
    }
  }
}
...
struct MergeStackElement {
  ...
  MergeStackElement(SceneNodeAlembicPtr node, SceneNodeAppPtr parent,
                    AppNodeMapPtr pcmap)
      : currFileNode(node), parentAppNode(parent), parentChildMapPtr(pcmap)
  {
  }
};

bool MergeSceneFile(SceneNodeAlembicPtr fileRoot, SceneNodeAppPtr appRoot,
                    const IJobStringParser& jobParams, CommonProgressBar* pbar,
                    std::list<SceneNodeAppPtr>* newNodes)
{
  ESS_PROFILE_FUNC();
  ...
  for (SceneChildIterator it = fileRoot->children.begin();
    ...
    //}
    sceneStack.push_back(
        MergeStackElement(fileNode, appRoot, buildChildMap(appRoot)));
  }
  ...
  while (!sceneStack.empty()) {
    ...
    // if matches a child of parent attach to it, and set "new" app node to the
    // matching node
    ...
    if (appNodeIt != childMapPtr->end()) {  // we have a match
      ...
      SceneNodeAlembicPtr ignoreFileNode;
      bool bChildAttached =
          appNode->replaceData(currFileNode, jobParams, ignoreFileNode);
        ...
        if (fileNode->isMerged()) {
          ...
          for (SceneChildIterator cit = mergedChild->children.begin();
            ...
            SceneNodeAlembicPtr cfileNode =
                reinterpret<SceneNode, SceneNodeAlembic>(*cit);
            sceneStack.push_back(
                MergeStackElement(cfileNode, newAppNode, childMapPtr));
          }
        }
        else {
          sceneStack.push_back(
              MergeStackElement(fileNode, newAppNode, childMapPtr));
        }
      }
