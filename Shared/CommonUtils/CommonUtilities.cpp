...
std::string getExporterName(std::string const& shortName)
{
  ...
  exporterName << "Exocortex Crate for " << shortNameSafe << ","
               << EC_QUOTE(crate_ver) << "," << EC_QUOTE(alembic_ver) << ","
               << EC_QUOTE(hdf5_ver);
...
std::string resolvePath(std::string const& originalPath)
{
  ...
  /*   static std::map<std::string,std::string> s_originalToResolvedPath;
     ...
     if( s_originalToResolvedPath.find( originalPath ) !=
     s_originalToResolvedPath.end() ) {
             return s_originalToResolvedPath[ originalPath ];
     }*/

  std::string resolvedPath =
      resolvePath_Internal(EnvVariables::replace(originalPath));

  // s_originalToResolvedPath.insert( std::pair<std::string,std::string>(
...
std::string addArchive(Alembic::Abc::IArchive* archive)
{
  ...
  gArchives.insert(
      std::pair<std::string, AlembicArchiveInfo>(archive->getName(), info));
...
void deleteAllArchives()
{
  ...
  for (std::map<std::string, AlembicArchiveInfo>::iterator it =
           gArchives.begin();
       it != gArchives.end(); ++it) {
    it->second.archive->reset();
    delete (it->second.archive);
...
Abc::IObject getObjectFromArchive(std::string const& path,
                                  std::string const& identifier)
{
                         ...
                         size_t numSamps)
{
  ...
  result.alpha =
      (iFrame - floorIndex.second) / (ceilIndex.second - floorIndex.second);
  return result;
}
...
{
  // Max Scene nodes are also identified by their transform nodes since an INode
  // contains
  // both the transform and the shape.  So if we find an "xfo" at the end of the
  // identifier
...
template <typename T>
inline bool isObjectSchemaConstant(T& schema)
{
...
template <typename T>
bool __getBasicSchemaDataFromObject(BasicSchemaData::SCHEMA_TYPE type,
                                    T& schema, BasicSchemaData& bsd)
{
  bsd.type = type;
                                  ...
                                  BasicSchemaData& bsd)
{
  ...
  const Alembic::Abc::MetaData& md = object.getMetaData();
  if (Alembic::AbcGeom::IXform::matches(md))
    return __getBasicSchemaDataFromObject(
        BasicSchemaData::__XFORM,
        Alembic::AbcGeom::IXform(object, Alembic::Abc::kWrapExisting)
            .getSchema(),
        bsd);
                              ...
                              SampleInfo const& sampleInfo)
{
  Alembic::Abc::TimeSamplingPtr timeSampling =
      getTimeSamplingFromObject(object);
...
void getMergeInfo(AbcArchiveCache* pArchiveCache, AbcObjectCache* pObjectCache,
                  ...
                  AbcObjectCache** ppMergedChildObjectCache)
{
  NodeCategory::type cat = NodeCategory::get(pObjectCache->obj);
  if (cat == NodeCategory::XFORM) {  // if a transform node, decide whether or
                                     // not use a dummy node OR merge this dummy
                                     // node with geometry node child

    unsigned geomNodeCount = 0;
    int mergeIndex;
    for (int j = 0; j < (int)pObjectCache->childIdentifiers.size(); j++) {
      AbcObjectCache* pChildObjectCache =
          &(pArchiveCache->find(pObjectCache->childIdentifiers[j])->second);
...
//      getMergeInfo(iObj, bCreateNullNode, nMergedGeomNodeIndex,
//      mergedGeomChild);
//
...
//         if( childCat == NodeCategory::UNSUPPORTED ) continue;// skip over
//         unsupported types
//
...
struct AlembicSelectionStackElement {
  ...
  AlembicSelectionStackElement(AbcObjectCache* pMyObjectCache, bool selected)
      : pObjectCache(pMyObjectCache), bSelected(selected)
  {
  }
};
...
Abc::ICompoundProperty AbcNodeUtils::getUserProperties(
    const AbcG::IObject& iObj, AbcA::TimeSamplingPtr& timeSampling,
    int& nSamples)
{
  if (AbcG::IXform::matches(iObj.getMetaData())) {
    AbcG::IXform obj(iObj, Abc::kWrapExisting);
...
void AbcNodeUtils::printCompoundProperty(Abc::ICompoundProperty prop)
{
  ...
  for (size_t i = 0; i < prop.getNumProperties(); i++) {
    ...
    ESS_LOG_WARNING("PropertyName: "
                    << pheader.getName() << ", pod: "
                    << AbcNodeUtils::getPodStr(pheader.getDataType().getPod())
                    << ", extent: " << (int)pheader.getDataType().getExtent()
                    << ", interpretation: "
...
Abc::UInt16ArraySamplePtr getCurveOrders(AbcG::ICurves& obj)
{
  ...
  if (arbGeom.getPropertyHeader(".orders") != NULL) {
    Abc::IUInt16ArrayProperty orders =
        Abc::IUInt16ArrayProperty(arbGeom, ".orders");
...
void clearIdentifierMap() { identifierCount.clear(); }
